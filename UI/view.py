...
class View:
    ...
    def load_interface(self):
        ...
        # Riga 2
        self.dd_album = ft.Dropdown(label="Album", width=200, on_change=self.controller.get_selected_album, disabled=True)
        self.pulsante_analisi_comp = ft.ElevatedButton(text="Analisi Componente",
                                                 on_click=self.controller.handle_analisi_comp,disabled=True)
        row2 = ft.Row([
        ...
        # Riga 3
        self.txt_durata_totale = ft.TextField(label="Durata Totale", disabled=True)
        self.pulsante_set_album = ft.ElevatedButton(text="Set di Album",
                                                    on_click=self.controller.handle_get_set_album, disabled=True)
        row3 = ft.Row([
